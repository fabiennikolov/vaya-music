            ...
            @foreach ($events as $event)
                <div
                    ...
                    <div class="md:order-1">
                        <img class="rounded-lg aspect-square object-cover bg-cover" src="{{$event->getFirstMediaUrl('event')}}" alt="Drama Queen Album Cover">
                    </div>
                    <div class="text-center mt-4 md:mt-0 md:order-2">
                        <p class="text-2xl text-[#45219F]">{{ \Carbon\Carbon::parse($event->date)->locale('bg')->isoFormat('D MMMM Y') }}</p>
